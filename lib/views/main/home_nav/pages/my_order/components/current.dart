...
class CurrentSection extends StatefulWidget {
  const CurrentSection({super.key});

  @override
  State<CurrentSection> createState() => _CurrentSectionState();
}

class _CurrentSectionState extends State<CurrentSection> {
  final bloc = KiwiContainer().resolve<CurrentOrderBloc>()..add(GetCurrentOrderEvent());
                        ...
                        Builder(
                          builder: (context) {
                            if(state.list[index].products.length< 3){
                              return Row(
                                children: List.generate(state.list[index].products.length, (i) =>
                                    SizedBox(
                                  height: 30.h,
                                  width: 30.w,
